...
module EVExport
  ...
  def self.command_213
    ...
    @arg = sprintf('%d (%s) on %s', @params[1],
      ...
      'Cobweb', 'Silence', 'Light Bulb', 'Zzz'][@params[1]],
      self.get_character(@params[0]))
    @arg += ' (wait)' if @params[2]
  end
  #--------------------------------------------------------------------------
  ...
  def self.command_284
    ...
    @arg += sprintf(' (loop horizontal%s)', 
      (@params[3] > 0 ? " [scroll " + @params[3] + "]" : "")) if @params[1]
    @arg += sprintf(' (loop vertical%s)', 
      (@params[4] > 0 ? " [scroll " + @params[4] + "]" : "")) if @params[2]
  end
  #--------------------------------------------------------------------------
  # * Get Location Info
  #--------------------------------------------------------------------------
  ...
  def self.command_319
    ...
    @arg += sprintf('%s %s %d %s',
      self.get_actor(0, @params[0] + 1), $data_system.terms.etypes[@params[1]],
  ...
  def self.command_324
    @text_export = true
    @cmd = "Change Nickname"
    @arg = sprintf('Actor %d (%s) to %s', @params[0], $data_actors[@params[0]],
      @params[1])
  end
  #--------------------------------------------------------------------------
  # * Change Enemy HP
